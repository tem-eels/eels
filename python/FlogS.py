...
def FlogS(infile=None, fwhm2=None):
    """
    Fourier-log deconvolution using exact methods (A) or (B) 
    Method for thicker samples (t/lambda > pi) as described by Spence (1979).
    """
    print("\n----------------FlogS---------------\n\n")

    # Read spectrum from input file
    if infile is None:
        print("Alternate Usage: FlogS('Filename', FWHM2)\n")
    ...
    nn = 2 ** (int(np.floor(np.log2(nd))) + 2)
    d = np.zeros(nn)
    z = np.zeros(nn)
    ...
    cosb = 0.5 - 0.5 * np.cos(np.pi * np.arange(nn//2-nfin+1) / (nn//2-nfin-nz-1))
    d[nfin-1:nn//2] = d[nfin-1] * ((nfin-1) / np.arange(nfin-1, nn//2)) ** r - cosb * dext
    d[nn//2-1:] = 0  # set right half of data to zero for thicker sample method
    ...
    dbyz = np.real(dbyz)  # use only real part for thicker sample method
    ...
    eout = epc * np.arange(nz, nn//2 + nz)
    dout = np.real(d[:nn//2])
    np.savetxt('FlogS.ssd', np.column_stack((eout, dout)), fmt='%8.15g %8.15g')
    ...
    plt.legend()
    plt.title('FLogS output', fontsize=12)
